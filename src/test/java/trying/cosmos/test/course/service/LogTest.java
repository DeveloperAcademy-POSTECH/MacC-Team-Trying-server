...
public class LogTest {
    ...
    class success {
        ...
        void log() throws Exception {
            ...
            em.persist(new CourseReview(user, courseYesterday, BODY));
            Course courseToday = courseRepository.save(new Course(planet, TITLE, LocalDate.now()));
            em.persist(new CourseReview(user, courseToday, BODY));
            Course courseTomorrow = courseRepository.save(new Course(planet, TITLE, LocalDate.now().plusDays(1)));

            // WHEN
            Slice<CourseFindResponse> logs = courseService.findLogs(user.getId(), pageable);

            // THEN
            List<Long> courseIds = logs.getContent().stream()
